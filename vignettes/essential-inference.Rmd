...
The `compare_models()` function produces the following output:

- `model`: an indicator of which model is being summarized
- `error.df`: the degrees of freedom associated with the error sum of squares
- `error.sumsq`: the error (or residual) sum of squares for the model
- `df`: the degrees of freedom associated with the hypothesis
- `sumsq`: the sum of squares treatment (regression) for the factor
- `statistic`: the F-statistic comparing the two models
- `p.value`: the p-value associated with the test

We notice the output is similar to that provided by the common `summary()` after running `aov()` or the `anova()` function in the `stats` package:
...
The difference is that the `aov()` approach is specific to ANOVA while `compare_models()` is not.  Both the `aov()` approach and the `anova()` approach are also limited to classical inference; `compare_models()` allows for inference via bootstrapping.
...
where $\mu$ represents the mean fuel efficiency (mpg) for all 1974 vehicles.  From a classical perspective, this is typically done using a one-sample t-test.  This can be implemented via `compare_models()`, but it does require some thought which is, we admit, not intuitive.
