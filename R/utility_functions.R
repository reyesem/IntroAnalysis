...
# Produces the error sum of squares for an arbitrary linear model with
#  responses generated under bootstrapping
my_SSE <- function(newy, fit) {
  .n <- nrow(newy)
  .w <- fit$weights
  ...
  # account for a model with no parameters
  if (!is.null(fit$qr)) {
    .P1 <- (qr.Q(fit$qr)) %*% t(qr.Q(fit$qr))

    return(colSums(newy * ((diag(.n) - .P1) %*% newy)))
  } else {
    return(colSums(newy^2))
  }
}
...
predict_coef <- function(object, newdata, beta = object$coefficients){
  ...
  predictor <- X[, piv, drop = FALSE] %*% as.matrix(beta)[piv, ]
